...
import (
	...
	"github.com/MadHive/taco/common/gizmo/auth"
...
func VerifyIdentityEmails(ctx context.Context, emails []string, audience string) auth.VerifyFunc {
	...
	return IdentityVerifyFunc(func(_ context.Context, cs IdentityClaimSet) bool {
		return strings.TrimPrefix(cs.Iss, "https://") == "accounts.google.com" &&
			cs.Aud == audience &&
			cs.EmailVerified &&
			emls[cs.Email]
	})
